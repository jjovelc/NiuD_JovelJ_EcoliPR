...
setwd('/Users/juanjovel/Library/CloudStorage/OneDrive-UniversityofCalgary/jj/UofC/data_analysis/dongyanNiu/GWAS_Ecoli/BICOSS')

# Read the final results
cat("=== READING FINAL BICOSS RESULTS ===\n")
results <- read.table("BICOSS_results_FINAL_gene_names_subset.txt", sep = "\t", header = TRUE, stringsAsFactors = FALSE)
...
ggsave("SNP_Trait_subset_Associations_Plot.png", 
       plot = p, 
       width = 6, 
       height = 5, 
       dpi = 300, 
...
ggsave("SNP_Trait_subset_Associations_Plot.pdf", 
       plot = p, 
       width = 12, 
       height = 8, 
       bg = "white")
...
ggsave("SNP_Trait_subset_Associations_Plot.svg", 
       plot = p, 
       width = 12, 
       height = 8, 
       bg = "white")
...
cat("Plots saved as:\n")
cat("- SNP_Trait_subset_Associations_Plot.png (high-res)\n")
cat("- SNP_Trait_subset_Associations_Plot.pdf (vector)\n")
cat("- SNP_Trait_subset_Associations_Plot.svg (publication)\n")
